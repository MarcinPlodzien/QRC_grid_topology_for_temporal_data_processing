...
@partial(jit, static_argnames=['L_chain', 'n_rails'])
def construct_hamiltonian_multirail(h_list, J_rails, J_rungs, L_chain, n_rails, field_rails):
    """
    Constructs dense Hamiltonian for Multi-Rail topology.
    Rails are indexed 0 to n_rails-1.
    Rail 0: Input Rail.
    Rails 1..K: Reservoir.
    
    Couplings:
        J_rails[k]: Intra-rail coupling for Rail k.
        J_rungs[k]: Coupling between Rail k and Rail k+1.
        
    Args:
        h_list (list/tuple): List of n_rails arrays, each of shape (L_chain,).
        J_rails (list/tuple): List of n_rails tuples (Jx, Jy, Jz).
        J_rungs (list/tuple): List of n_rails-1 tuples (Jx, Jy, Jz).
        L_chain (int): Length of one rail.
        n_rails (int): Number of rails.
        field_rails (list/tuple): List of fields.
    """
    ...
    for r in range(n_rails):
        fx, fy, fz = field_rails[r]
        h_vec = h_list[r] # Shape L
        
        for i in range(L_chain):
            global_idx = r * L_chain + i
            op = h_vec[i] * (fx*X + fy*Y + fz*Z)
            H += get_term([op], [global_idx])
            
    # 2. Intra-Rail Couplings
    for r in range(n_rails):
        Jx, Jy, Jz = J_rails[r]
        offset = r * L_chain
        
        for i in range(L_chain - 1):
            idx1, idx2 = offset + i, offset + i + 1
            H += Jx * get_term([X, X], [idx1, idx2])
            H += Jy * get_term([Y, Y], [idx1, idx2])
            H += Jz * get_term([Z, Z], [idx1, idx2])
            
    # 3. Inter-Rail (Rung) Couplings (Between r and r+1)
    # Assumes rungs connect site i on rail r to site i on rail r+1
    for r in range(n_rails - 1):
        Jx, Jy, Jz = J_rungs[r]
        offset_r = r * L_chain
        offset_next = (r + 1) * L_chain
...
@partial(jit, static_argnames=['L_chain', 'n_rails'])
def construct_hamiltonian_multirail_sparse(h_list, J_rails, J_rungs, L_chain, n_rails, field_rails):
    """
    Constructs sparse BCOO Hamiltonian for Multi-Rail topology.
    """
    N_total = n_rails * L_chain
    dim = 2**N_total
    ...
    for r in range(n_rails):
        ...
        for i in range(L_chain):
            idx = offset + i
            add_term(h_vec[i] * fx, ['X'], [idx])
            add_term(h_vec[i] * fy, ['Y'], [idx])
            add_term(h_vec[i] * fz, ['Z'], [idx])
    ...
    for r in range(n_rails):
        ...
        offset = r * L_chain
        for i in range(L_chain - 1):
            s1, s2 = offset + i, offset + i + 1
            add_term(Jx, ['X','X'], [s1, s2])
            add_term(Jy, ['Y','Y'], [s1, s2])
            add_term(Jz, ['Z','Z'], [s1, s2])
...
def partial_trace_and_reset(rho_total, psi_new_L, L_chain):
    ...
    # Trace means summing diagonal elements of L subsystem.
