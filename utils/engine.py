...
def measure_observables(rho_total, L_chain, observables):
    ...
    """
    dim_L = 2**L_chain
    dim_R = rho_total.shape[0] // dim_L
    L_R = int(jnp.log2(dim_R)) # Number of reservoir qubits
    ...
    for req in observables:
        ...
        if 'Z_' in req and 'ZZ_' not in req:
            ...
            for i in range(L_R):
                ...
                mask = (jnp.arange(dim_R) >> (L_R - 1 - i)) & 1
                # Z val: +1 if 0, -1 if 1
            ...
            if req == 'Z_local_mean':
                ...
                obs_list.append(jnp.array([z_tot / L_R]))
                ...
                for i in range(L_R):
                    for j in range(i + 1, L_R):
                         mask_i = (jnp.arange(dim_R) >> (L_R - 1 - i)) & 1
                         mask_j = (jnp.arange(dim_R) >> (L_R - 1 - j)) & 1
        ...
        # --- X Family (Off-Diagonal) ---
            ...
            if req == 'X_local_mean':
                ...
                obs_list.append(jnp.array([x_tot / L_R]))
        ...
        # --- Y Family (Complex Off-Diagonal) ---
            ...
            if req == 'Y_local_mean':
                ...
                obs_list.append(jnp.array([y_tot / L_R]))
        ...
        # --- ZZ Correlation (Nearest Neighbor) ---
             ...
             zz_locals = []
             for i in range(L_R - 1):
                mask_i = (jnp.arange(dim_R) >> (L_R - 1 - i)) & 1
                mask_j = (jnp.arange(dim_R) >> (L_R - 1 - (i+1))) & 1
             ...
             if req == 'ZZ_local_mean':
                 ...
                 exp_sq = jnp.array(L_R - 1, dtype=jnp.float64) 
...
@partial(jit, static_argnames=['L_chain', 'n_rails', 'measured_rails', 'observables'])
def measure_observables_multirail(rho_total, L_chain, n_rails, measured_rails, observables):
    """
    Generalized measurement function for Multi-Rail systems.
    
    Allows selecting specific rails to measure via `measured_rails`.
    Measurements are taken on the FULL state `rho_total` (no partial trace performed here).
    
    Args:
        rho_total: density matrix (2^Ntotal, 2^Ntotal)
        L_chain: sites per rail
        n_rails: total rails
        measured_rails: tuple of integers (e.g. (0, 1, 2) or (2,))
        observables: tuple of strings
        
    Returns:
        jnp.ndarray: Concatenated measurements.
    """
    N_total = n_rails * L_chain
    dim = 2**N_total

    # Pre-extract Diagonal for Z measures (Optimization)
    diag = jnp.diag(rho_total).real
    
    obs_list = []
    
    # Helper to map (rail, site) to global index
    # Rails are 0..N-1. Sites 0..L-1.
    # Global = rail * L + site
    ...
    for req in observables:
        ...
        if 'Z_' in req and 'ZZ_' not in req:
            z_locals = []
            for r in measured_rails:
                offset = r * L_chain
                fori = range(L_chain)
                for i in range(L_chain):
                    idx = offset + i
                    mask = (jnp.arange(dim) >> (N_total - 1 - idx)) & 1
                    val = jnp.where(mask == 0, 1.0, -1.0)
                    z_locals.append(jnp.sum(diag * val))
            
            z_arr = jnp.array(z_locals)
            
            if req == 'Z_local_mean': obs_list.append(z_arr)
            elif req == 'Z_total_mean': obs_list.append(jnp.array([jnp.mean(z_arr)]))
            elif req == 'Z_local_std': obs_list.append(jnp.sqrt(jnp.maximum(1.0 - z_arr**2, 0.0)))
            elif req == 'Z_total_std': 
                # Approx (no correlations for now to save compute)
                obs_list.append(jnp.array([0.0]))
        ...
        # --- X Family ---
            ...
            for r in measured_rails:
                ...
                for i in range(L_chain):
                    idx = offset + i
                    us = jnp.arange(dim)
                    vs = us ^ (1 << (N_total - 1 - idx))
                    x_locals.append(jnp.sum(rho_total[us, vs]).real)
            
            x_arr = jnp.array(x_locals)
            
            if req == 'X_local_mean': obs_list.append(x_arr)
            elif req == 'X_total_mean': obs_list.append(jnp.array([jnp.mean(x_arr)]))
            elif req == 'X_local_std': obs_list.append(jnp.sqrt(jnp.maximum(1.0 - x_arr**2, 0.0)))
        ...
        # --- Y Family ---
            ...
            for r in measured_rails:
                ...
                for i in range(L_chain):
                    ...
                    us = jnp.arange(dim)
                    vs = us ^ (1 << (N_total - 1 - idx))
                    bit_val = (us >> (N_total - 1 - idx)) & 1
                    # Y: 0->1 (-i), 1->0 (i)
                    phase = jnp.where(bit_val == 0, -1j, 1j)
                    y_locals.append(jnp.sum(rho_total[us, vs] * phase).real)
            
            y_arr = jnp.array(y_locals)
            if req == 'Y_local_mean': obs_list.append(y_arr)
            elif req == 'Y_total_mean': obs_list.append(jnp.array([jnp.mean(y_arr)]))
            elif req == 'Y_local_std': obs_list.append(jnp.sqrt(jnp.maximum(1.0 - y_arr**2, 0.0)))


        # --- ZZ Correlations (Intra-Rail only for now) ---
        elif 'ZZ_' in req:
             zz_locals = []
             for r in measured_rails:
                 offset = r * L_chain
                 # NN neighbors in this rail
                 for i in range(L_chain - 1):
                     idx1 = offset + i
                     idx2 = offset + i + 1
                     mask1 = (jnp.arange(dim) >> (N_total - 1 - idx1)) & 1
                     mask2 = (jnp.arange(dim) >> (N_total - 1 - idx2)) & 1
                     val = 1.0 - 2.0 * (mask1 ^ mask2)
                     zz_locals.append(jnp.sum(diag * val))
             
             zz_arr = jnp.array(zz_locals) if len(zz_locals) > 0 else jnp.array([0.0])
             if req == 'ZZ_local_mean': obs_list.append(zz_arr)
         ...
         # --- XX Correlations ---
             ...
             for r in measured_rails:
                 ...
                 for i in range(L_chain - 1):
                     ...
                     idx2 = offset + i + 1
                     us = jnp.arange(dim)
                     vs = us ^ (1 << (N_total-1-idx1)) ^ (1 << (N_total-1-idx2))
                     xx_locals.append(jnp.sum(rho_total[us, vs]).real)
             
             xx_arr = jnp.array(xx_locals) if len(xx_locals) > 0 else jnp.array([0.0])
             if req == 'XX_local_mean': obs_list.append(xx_arr)
         ...
         # --- YY Correlations ---
             ...
             for r in measured_rails:
                 ...
                 for i in range(L_chain - 1):
                     ...
                     us = jnp.arange(dim)
                     vs = us ^ (1 << (N_total-1-idx1)) ^ (1 << (N_total-1-idx2))
                     bit1 = (us >> (N_total - 1 - idx1)) & 1
                     bit2 = (us >> (N_total - 1 - idx2)) & 1
                     fact1 = jnp.where(bit1 == 0, -1j, 1j)
                     fact2 = jnp.where(bit2 == 0, -1j, 1j)
                     yy_locals.append(jnp.sum(rho_total[us, vs] * fact1 * fact2).real)
             
             yy_arr = jnp.array(yy_locals) if len(yy_locals) > 0 else jnp.array([0.0])
             if req == 'YY_local_mean': obs_list.append(yy_arr)
...
@partial(jit, static_argnames=['L_chain', 't_evol', 'dt', 'h_mag', 'n_rails', 'topology', 'target_input_state', 'integration_method', 'observables', 'prepare_measurement', 'field_disorder', 'measured_rails'])
def simulation_kernel_multirail(key_seed, input_batch, L_chain, t_evol, dt, h_mag, 
                                J_rails, J_rungs, field_rails, n_rails,
                                target_input_state, topology, measured_rails, integration_method='trotter', 
                                observables=('Z',), prepare_measurement=True, field_disorder=True):
    """
    Simulation Kernel for Multi-Rail Topology with Flexible Measurement.
    """
    n_trotter = int(t_evol / dt)
    
    # 1. Disorder (Fields Only)
    keys_rails = jax.random.split(key_seed, n_rails)
    
    h_list = []
    if field_disorder:
        for r in range(n_rails):
            h_vec = h_mag * jax.random.uniform(keys_rails[r], (L_chain,), minval=-0.5, maxval=0.5)
            h_list.append(h_vec)
    else:
        for r in range(n_rails):
            h_vec = jnp.full((L_chain,), h_mag, dtype=jnp.float64)
            h_list.append(h_vec)
            
    # 2. Init State
    n_res_qubits = (n_rails - 1) * L_chain
    dim_R = 2**n_res_qubits
    
    psi_R = jnp.zeros(dim_R, dtype=jnp.complex128).at[0].set(1.0)
    rho_R = jnp.outer(psi_R, psi_R.conj())
    ...
    if integration_method == 'exact_eig':
        H_static = uqc.construct_hamiltonian_multirail(
            h_list, J_rails, J_rungs, L_chain, n_rails, field_rails
        )
        eig_vals, eig_vecs = jnp.linalg.eigh(H_static)
        U_step = uti.construct_unitary_eig(eig_vals, eig_vecs, t_evol)
        U_step_dag = jnp.conj(U_step).T
        ...
        H_sparse = uqc.construct_hamiltonian_multirail_sparse(
            h_list, J_rails, J_rungs, L_chain, n_rails, field_rails
        )
    ...
    def time_step(rho_curr, u_vec_t):
        psi_new_L = prep_input_state_vectors(u_vec_t, L_chain, target_input_state)
        # partial_trace generic handles N-rail
        rho_injected = uqc.partial_trace_and_reset(rho_curr, psi_new_L, L_chain)
        ...
        if integration_method == 'exact_eig':
             ...
             rho_evolved = uti.evolve_rk4_sparse(rho_injected, H_sparse, dt, n_trotter)
             
        elif integration_method == 'rk4_dense':
             if H_static is None:
                  H_mat = uqc.construct_hamiltonian_multirail(h_list, J_rails, J_rungs, L_chain, n_rails, field_rails)
             else:
                  H_mat = H_static
             rho_evolved = uti.evolve_rk4_dense_operator(rho_injected, H_mat, dt, n_trotter)
        ...
        if prepare_measurement:
             # Use NEW Measure Function with Measured Rails Tuple
             obs = measure_observables_multirail(rho_evolved, L_chain, n_rails, measured_rails, observables)
             return rho_evolved, obs
        else:
             return rho_evolved, jnp.array([0.0])
